...
# Configuration
CONFIG = {
    'SYMBOLS_FILE': 'symbolsall.csv',
    'RAW_DATA_FILE': 'raw_data_all.csv',
    'ADJUSTED_DATA_FILE': 'data_all.csv',
    'SPLITS_LOG_FILE': 'detected_splits_all.csv',
    'ANALYSIS_OUTPUT_FILE': 'snapshot_all.csv',
    'MAX_WORKERS': 6,
...
def setup_logging(verbose=True):
    ...
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler('workflow_all.log'), logging.StreamHandler()]
    )
...
# Modified get_nse_holiday_dates to use holidays from symbolsall.csv
...
def fetch_and_format(trade_date, symbol_filter=None, logger=None):
    try:
        ...
        # Standardize symbols by stripping suffixes to match the loaded symbol list
        spot_ohlcv['symbols'] = spot_ohlcv['symbols'].str.upper().str.strip().str.replace('.NS', '', regex=False).str.replace('-EQ', '', regex=False)
        ...
        if symbol_filter is not None:
            pre_filter_count = len(spot_ohlcv)
            spot_ohlcv = spot_ohlcv[spot_ohlcv['symbols'].isin(symbol_filter)]
            if logger:
                logger.info(f"Filtered {trade_date}: {len(spot_ohlcv)} matches found out of {pre_filter_count} market records.")
