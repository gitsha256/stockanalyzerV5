...
def main():
    ...
    if not user_input:
        ...
        def get_file_date(fname):
            ...
            try:
                # Filenames produced by analyzer start with DD-MM-YY (first 8 chars)
                return datetime.strptime(base[:8], "%d-%m-%y")
            except ValueError:
                # Fallback to modification time if filename format is unusual
